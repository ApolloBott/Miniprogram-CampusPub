  ...
  computed: {
    ...
    historyList() {
      switch(this.activeTab) {
        case 0: return this.collectedList;
        default: return [];
      }
    },
  },
  ...
  methods: {
    ...
    async loadHistory() {
      try {
		  ...
		  switch(this.activeTab)  {
			  case 0: const { data: collectData } = await uni.$http.get('/users/collect', queryObj);
			  res = collectData;
			  break;  
		  }
        if (res.meta.status === 200) {
          switch(this.activeTab) {
            case 0: this.collectedList = res.data; break;
          }
        }
      } catch (error) {
	...
	async getHistory() {
		try {
			...
			switch(this.activeTab) {
				case 0: const { data: collectData1 } = await uni.$http.get('/goods/historysearch', queryObj);
				res = collectData1;
			...
			if (res.meta.status === 200) { 
				switch(this.activeTab) {
				  case 0: this.collectedList = res.message; break;
				}
			}
		} catch (error) {
