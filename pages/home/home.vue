...
selectCategory(category) {
    ...
    this.loadPostList()
			...
			async loadPostList() {
			    try {
			        ...
			        if (res.meta.status === 200) {
			            ...
			            // 🔥 关键修改：统一的 hasMore 判断逻辑
			            if (this.sortMode === 'discover') {
			                // "发现"模式：只要后端返回了数据，就继续加载（即使为0也保持true用于触发循环）
			                this.hasMore = true
			                
			                // 🔥 存储本次返回的数量，供 loadMore() 使用
			                this._lastLoadCount = actualNewPostsCount
			                
			                console.log(`✅ 发现模式 - 本次返回: ${actualNewPostsCount}，总计: ${this.postList.length}`)
			            } else {
			                // "最新"和"关注"模式：使用服务器返回的 hasMore
			...
			// ✅ 修改：上拉加载更多
		async loadMore() {
		    if (this.isReachingBottom || this.isLoading) {
